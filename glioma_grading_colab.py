...
@dataclass
class Config:
    csv_path: str = "/content/glioma_grading.csv"
    ...
    output_dir: str = "/content/pr_ashdp_glioma_outputs"
...
def _find_target_column(df: pd.DataFrame) -> str:
    candidate_names = ["Grade", "grade", "GRADE", "Class", "class", "target", "Target", "Glioma_Grade", "glioma_grade"]
    ...
    for col in df.columns:
        ...
        if {"LGG", "GBM"}.issubset(uniq) or uniq.issubset({"0", "1", "LGG", "GBM", "HGG"}):
            return col
    raise ValueError("Could not detect target column. Rename it to Grade, Class, target, or Glioma_Grade.")


def _binarize_target(y_series: pd.Series) -> np.ndarray:
    s = y_series.astype(str).str.strip().str.upper()
    mapping = {
        "GBM": 1, "HGG": 1, "HIGH": 1, "HIGH_GRADE": 1, "4": 1, "1": 1,
        "LGG": 0, "LOW": 0, "LOW_GRADE": 0, "2": 0, "3": 0, "0": 0,
    }
    if set(s.dropna().unique()).issubset(set(mapping.keys())):
        y = s.map(mapping).astype(float).to_numpy()
        return y.astype(np.float32)
    numeric = pd.to_numeric(y_series, errors="coerce")
    if numeric.notna().mean() > 0.8:
        uniq = sorted(numeric.dropna().unique().tolist())
        if len(uniq) == 2:
            high = uniq[-1]
            return (numeric.to_numpy() == high).astype(np.float32)
    raise ValueError("Could not convert the glioma target to binary. Expected values like GBM/LGG or a binary numeric target.")


def load_glioma_dataset(csv_path: str, positive_label: int = 1):
    if os.path.exists(csv_path):
        print(f"Loading local glioma CSV: {csv_path}")
        ...
        print("Trying to fetch the UCI Glioma dataset with ucimlrepo...")
        X_df, y_series = _try_fetch_ucirepo_glioma()
        if X_df is None or y_series is None:
            raise FileNotFoundError("Glioma dataset not found locally and automatic fetch failed. Upload /content/glioma_grading.csv.")
        y = _binarize_target(pd.Series(y_series))

    X_df = X_df.replace(["?", "NA", "N/A", ""], np.nan)
...
def main(cfg: Config):
    ...
    seed_everything(42)
    X, y = load_glioma_dataset(cfg.csv_path, cfg.positive_label)

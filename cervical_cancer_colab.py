...
@dataclass
class Config:
    csv_path: str = "/content/risk_factors_cervical_cancer.csv"
    target_col: str = "Biopsy"
    ...
    output_dir: str = "/content/pr_ashdp_cervical_outputs"
...
def _try_fetch_ucirepo_cervical():
    try:
        ...
        try:
            ...
            from ucimlrepo import fetch_ucirepo
        ds = fetch_ucirepo(id=383)
        X = ds.data.features.copy()
        y = ds.data.targets.copy()
        return X, y
    except Exception as e:
        print("Automatic UCI fetch failed:", repr(e))
        return None, None


def _standardize_column_lookup(columns):
    return {str(c).strip().lower(): c for c in columns}


def _coerce_binary_series(s: pd.Series, positive_label: int = 1) -> np.ndarray:
    txt = s.astype(str).str.strip().replace({"?": np.nan, "nan": np.nan, "None": np.nan})
    num = pd.to_numeric(txt, errors="coerce")
    if num.notna().mean() > 0.5:
        return (num.fillna(0).to_numpy() == positive_label).astype(np.float32)
    upper = txt.str.upper()
    mapping = {
        "YES": 1, "NO": 0,
        "TRUE": 1, "FALSE": 0,
    ...
    mapped = upper.map(mapping)
    if mapped.notna().mean() > 0.5:
        return mapped.fillna(0).to_numpy(dtype=np.float32)
    raise ValueError("Could not convert target column to a binary numeric array.")


def load_cervical_dataset(csv_path: str, target_col: str, positive_label: int = 1):
    if os.path.exists(csv_path):
        print(f"Loading local cervical cancer CSV: {csv_path}")
        df = pd.read_csv(csv_path)
    else:
        print(f"Local file not found at: {csv_path}")
        print("Trying to fetch the UCI Cervical Cancer Risk Factors dataset with ucimlrepo...")
        X_df, target_df = _try_fetch_ucirepo_cervical()
        if X_df is None or target_df is None:
            raise FileNotFoundError(
                "Cervical cancer dataset not found locally and automatic fetch failed. "
                "Upload a CSV to /content/risk_factors_cervical_cancer.csv."
            )
        df = pd.concat([X_df, target_df], axis=1)

    col_lookup = _standardize_column_lookup(df.columns)
    key = target_col.strip().lower()
    if key not in col_lookup:
        available_targets = [c for c in df.columns if str(c).strip().lower() in {"hinselmann", "schiller", "cytology", "biopsy"}]
        raise ValueError(f"Target column '{target_col}' not found. Available diagnostic targets: {available_targets}")

    target_real_col = col_lookup[key]
    y = _coerce_binary_series(df[target_real_col], positive_label=positive_label)
    X_df = df.drop(columns=[target_real_col]).copy()

    leakage_targets = {"hinselmann", "schiller", "cytology", "biopsy"}
    drop_cols = [c for c in X_df.columns if str(c).strip().lower() in leakage_targets]
    if drop_cols:
        X_df = X_df.drop(columns=drop_cols)

    X_df = X_df.replace(["?", "NA", "N/A", "", "nan", "None"], np.nan)
    ...
    if len(num_cols) > 0:
        ...
        X_df[num_cols] = X_df[num_cols].fillna(X_df[num_cols].median(numeric_only=True))
    ...
    y = pd.Series(y).fillna(0).to_numpy(dtype=np.float32)
...
def main(cfg: Config):
    ...
    seed_everything(42)
    X, y = load_cervical_dataset(cfg.csv_path, cfg.target_col, cfg.positive_label)
